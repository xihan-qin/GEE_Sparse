...
class plot_graph:
    ...
    def four_plots(self, A,y,bl,pp, fig_name):
        """
            reference: https://matplotlib.org/stable/gallery/subplots_axes_and_figures/demo_tight_layout.html
        """       
        A_reorder, block_idx = re_order_by_label(A, y)
...
def plot_block_probabilities(Bl, ax=None):
    """
        ...
        https://stackoverflow.com/questions/69257255/change-colorbar-limits-without-changing-the-values-of-the-data-it-represents-in        
    """
    if ax is None:
    ...
    ax.set_xticks(np.arange(block_num), labels = block_labels)
    ax.set_yticks(np.arange(block_num), labels = block_labels) 
    ...
    ax.set_title("Blcok probabilities")

    # create colorbar
    cbar = ax.figure.colorbar(im, cmap="Greys")
    cbar.ax.set_ylabel("block_probabilities", rotation=-90, va="bottom")
...
def plot_reordered_Adj(A_reorder, block_idx, ax=None):
    ...
    # set the limits of the plot to the limits of the data
    ax.set_xticks(label_idx, labels = block_labels)
    ax.set_yticks(label_idx, labels = block_labels)     
    ...
    ax.set_title("Adjacnecy Matrix reordered by node labels")
...
def plot_labels(Y, Bl, ax= None):
    ...
    ax.set_title("Community Label vs Node Count")
...
def label_pp_pie(Bl, pp, ax = None):
    ...
    ax.set_title("Initial Community label percentage before sampling")
# ---------------------labels_pie_end------------------------------------------#
...
def try_different_interpolation_methods(grid):
    ...
    for ax, interp_method in zip(axs.flat, methods):
      ax.imshow(grid, interpolation=interp_method, cmap='binary') # for choosing colors: https://matplotlib.org/stable/tutorials/colors/colormaps.html
...
def auto_plot(plot_function):
    """
        ...
        conditional reference: https://stackoverflow.com/questions/10724854/how-to-do-a-conditional-decorator-in-python
        global reference: https://stackoverflow.com/questions/30563807/apply-a-single-decorator-to-multiple-functions
    """
